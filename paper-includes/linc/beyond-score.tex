...
we provided insights into the strengths and weaknesses of both approaches. Overall, we found that they had distinct behaviors and
...
\textbf{L3: FOL contains syntax errors.}  Across all generations, we found that the FOL expressions sometimes contained syntax errors: 38\% for StarCoderPlus, 24\% for GPT-3.5, and 13\% for GPT-4. The most common error was that the same symbol was used with multiple arities. As an example, if \texttt{Summer(July4)} and \texttt{Loves(Alex, Summer)} were both present in a FOL translation, \texttt{Summer} would have a multiple arity violation.
...
\textbf{1. Compared to \texttt{CoT}, \texttt{LINC} had worse recall but better precision on \texttt{True}/\texttt{False} predictions.} To see this,
...
Therefore, \texttt{LINC}'s precision when predicting \texttt{True} or \texttt{False} was very high (93\%) compared to that of \texttt{CoT} (81\%), but this came at the cost of lower recall on \texttt{True}/\texttt{False} predictions (60\% for \texttt{LINC} vs. 75\% for \texttt{CoT}).
...
Through this work, we first saw by looking at the score that \texttt{LINC} outperformed CoT on
ProofWriter but performed similarly on FOLIO. However, through deeper analysis, we learned that
the failure modes of the two were completely different, making them complementary. CoT failed through natural-language reasoning errors, such as invalid deductions or final labels that
did not match the preceding reasoning. \texttt{LINC} avoided many of these reasoning errors by using
a theorem prover, but introduced a different bottleneck: lossy or invalid formalization into first-order
logic. Looking only at the score would have missed this distinction.
