...
We opt for \texttt{StarCoder+} for three reasons: firstly, unlike the other models we consider, it is a free, open-access model. Secondly, it has a dataset search functionality (see \url{https://huggingface.co/spaces/bigcode/in-the-stack} and \url{https://huggingface.co/spaces/bigcode/search}), with which we verify that FOLIO and ProofWriter are not in \texttt{StarCoder+}'s training set, giving further assurance in the validity of our findings. Thirdly, with its 15.5B parameters it is likely considerably smaller than GPT-3.5 and GPT-4; although the exact size of these models has not been made public, their common predecessor GPT-3 was known to have 175B parameters \citep{brown2020language}. This allows us to compare performance at different model scales.
