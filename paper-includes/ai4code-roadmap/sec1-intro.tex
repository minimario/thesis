...
Moving forward to Sec. \ref{sec:challenges}, we highlight nine challenges in the field that today's models face, each cross-cutting and applicable to several tasks. In Sec. \ref{sec:paths}, we posit a set of promising research directions to tackle the challenges above, with Fig. \ref{fig:challenges-paths} summarizing which directions correspond to each challenge. We hope that through this chapter, the reader can appreciate the progress the field has made, understand the shortcomings of today's state-of-the-art models, and take inspiration from our suggested future ideas for tackling these challenges.
...
\begin{figure}
    \centering
    \begin{tikzpicture}[
        every node/.style={draw, text width=5cm, align=center, rounded corners, fill=blue!20, minimum height=1cm, inner sep=3pt},
        challenge/.style={fill=lightblue},
        solution/.style={fill=lightorange},
        line/.style={draw, -stealth, thin}
    ]
        % Adjusted vertical spacing for better readability
        \node[challenge] (C6) at (0, 6.5) {Semantic Understanding \\ of Codebases};
        \node[challenge] (C1) at (0, 5.2) {Evaluation and Benchmarks};
        \node[challenge] (C3) at (0, 3.9) {Human-AI Collaboration};
        \node[challenge] (C7) at (0, 2.6) {Low-Resource Languages and Specialized Libraries};
        \node[challenge] (C8) at (0, 1.3) {Library and API Version Updates};
        \node[challenge] (C2) at (0, 0) {Effective Tool Usage};
        \node[challenge] (C9) at (0, -1.3) {High Logical Complexity \\ and OOD Domains};
        \node[challenge] (C4) at (0, -2.6) {Long-Horizon Code Planning};
        \node[challenge] (C5) at (0, -3.9) {Large Scope and Long Contexts};
        % Solutions (Right Side)
        \node[solution] (S1) at (6.0, 6.5) {Automatic Data Curation};
        \node[solution] (S7) at (6.0, 5.2) {Human-Centric \\ Data Curation};
        \node[solution] (S8) at (6.0, 3.9) {Training Models to \\ Collaborate with Humans};
        \node[solution] (S9) at (6.0, 2.6) {Scaffolding Human \\ Supervision};
        \node[solution] (S3) at (6.0, 1.3) {Fast Specialized \\ Codebase Adaptation};
        \node[solution] (S6) at (6.0, 0) {Incorporating SWE Tools};
        \node[solution] (S2) at (6.0, -1.3) {Reinforcement Learning for Code};
        \node[solution] (S5) at (6.0, -2.6) {Integration with SWE \\ Development Frameworks};
        \node[solution] (S4) at (6.0, -3.9) {Semantic-Aware \\ Embeddings and Retrieval};

        % Connections
        \draw[line] (C1.east) -- ([yshift=-0.1cm]S1.west);
        \draw[line] (C1.east) -- (S7.west);
        \draw[line] (C2.east) -- (S6.west);
        \draw[line] (C3.east) -- ([yshift=-0.1cm]S7.west);
        \draw[line] (C3.east) -- (S8.west);
        \draw[line] (C3.east) -- (S9.west);
        \draw[line] (C4.east) -- ([yshift=-0.1cm]S2.west);
        \draw[line] (C4.east) -- (S5.west);
        \draw[line] (C5.east) -- (S4.west);
        \draw[line] (C5.east) -- ([yshift=-0.1cm]S5.west);
        \draw[line] (C6.east) -- (S1.west);
        \draw[line] (C7.east) -- ([yshift=0.1cm]S3.west);
