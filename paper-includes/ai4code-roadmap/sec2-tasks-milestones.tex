...
LiveCodeBench is a useful example of how a benchmark can instantiate this taxonomy while also revealing its limits. It is still largely at function-level scope, but it expands beyond direct code generation into self-repair, code execution, and test-output prediction. It also shows why benchmark freshness matters: by organizing problems by release date, the same evaluation can expose contamination effects and changing model ranks across time windows. In the taxonomy here, this means a benchmark should be described not only by task type and difficulty, but also by whether it isolates the capability under study from confounders such as benchmark age, overfitting, and prior exposure.

Next, with our taxonomy of measures in place, we turn to the set of tasks that are reflective of the tasks and capabilities of a human software engineer. We give a brief description of each task in this section.
...
\begin{aicodebluebox}
\textit{Example: Twitter migration to improve latency}: Twitter (\url{https://www.infoq.com/news/2012/11/twitter-ruby-to-java/}) built its initial platform using Ruby on Rails, facilitating rapid development. However, as the user base expanded, performance and scalability issues arose. They migrated key components to Java and Scala, achieving a 3X latency drop. This transition required re-architecting the system to adapt Ruby's dynamic features to the statically typed environments of Java and Scala, exemplifying the complexities of large-scale code translation.
\end{aicodebluebox}

\begin{aicodebluebox}
\textit{Example: C to Rust}: There has been a push to use translation as a proactive approach to eliminate memory safety vulnerabilities in C-based systems. This has even garnered attention from the US Department of Defense (\url{https://www.darpa.mil/news/2024/memory-safety-vulnerabilities}), which has long-lived systems that disproportionately depend on C, supporting programs to translate C codebases to Rust (\href{https://www.darpa.mil/research/programs/translating-all-c-to-rust}{TRACTOR}). 
% 
