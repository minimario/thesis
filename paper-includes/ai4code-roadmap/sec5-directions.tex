...
% \paragraph{Diverse Data Distribution}
...
\paragraph{Automatic Data Curation} \label{sec:d-automatic-data}
...
\paragraph{Human-Centric Data Curation} \label{d:subsec-human-data}
...
\paragraph{Environment Design for Code RL} \label{sec:d-rl}
\begin{aicodeorangebox}
...
% \paragraph{Post-Training: Distillation}
...
\paragraph{Adapting to Specialized and Quickly Changing Codebases} \label{sec:d-adaptation}
...
%\paragraph{Adapting to Interactive Setup}
\paragraph{Training Code LLMs to Collaborate with Humans}
...
\begin{aicodebluebox}
\textit{Example: Incomplete specification in Verus}: Here, we show a failure mode of LLMs when writing specifications and proofs in Verus. The LLM is asked to write the \texttt{ensures} postcondition clause for a ring buffer enqueue function (full example \href{https://github.com/microsoft/verus-proof-synthesis/blob/main/benchmarks/interprocedural/tock/verified/rb1.rs\#L270-L297}{here}). Here, the postcondition is incomplete: it does not check, for example, that the original elements were maintained in the ring buffer.
...
\begin{aicodebluebox}
\textit{Example}: For instance, in a release of \href{https://sakana.ai/ai-cuda-engineer/}{AI CUDA Engineer} by Sakana AI, an AI-generated CUDA kernel for lower triangular matrix multiplication---purportedly achieving significant speedups---was later found to exploit out-of-bounds memory access to bypass correctness checks (the full LLM-generated kernel code appears in Listing 3, pp.~46--47 of \cite{lange2025ai}). Advancing research on frameworks that facilitate test generation and automated adversarial testing represents an important direction.
\end{aicodebluebox}
...
\paragraph{Semantic-Aware Embeddings and Retrieval} \label{sec:directions-retrieval}
...
% \subsubsection{Training: New Objectives and Synthetic Data}
...
\paragraph{Integration with SWE Development Frameworks} \label{sec:d-swe-integration}
...
\paragraph{Incorporating SWE Tools} \label{d:sec-agents-tools}
...
\paragraph{Scaffolding Human Supervision}
...
%\subsubsection{New Human-AI Paradigms}
...
% \subsubsection{Developing a World Model for Code}
