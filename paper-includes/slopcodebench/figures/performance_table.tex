\begin{table}[H]
\centering
\caption{Per-model performance on \scb{}. For each model we report its best just-solve run, ranked by isolated solve rate with core, strict, and \$/checkpoint as tiebreakers. Solve rates use a fixed 196-checkpoint denominator; missing checkpoints count as unsolved. Other metrics are only for ran checkpoints. \textbf{Bold} marks the best value per column.}
